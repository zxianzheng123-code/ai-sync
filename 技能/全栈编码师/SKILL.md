...
> - [决策-计划协议](../决策-计划协议.md) — 决策框架、验收标准、SubAgent分派、进度管理
...
### 通用规则（跨阶段）
1. **Big Rewrite 原则**：发现方向不对时，先告知用户「老板，我发现 [具体问题]，继续修补不如推倒重写，因为 [理由]。行吗？」用户确认后才推倒从零写。
2. **Golden Hammer 原则**：能用同一套技术/模式解决的，禁止引入第二套。如果遇到不得不引入第二套的情况，先告知用户理由并确认。

### Phase A 特有
3. **用户零技术决策**：禁止向用户抛出技术选择题。技术决策由专家组内部完成。
4. **业务现状必须摸清**：在技术互撕之前，必须通过大白话问答摸清用户业务现状。禁止脑补。
5. **AI 友好优先**：多个方案同等可行时，优先选择 AI 写代码最容易理解和维护的方案。
6. **干净骨架**：初始化的项目必须是干净空项目，不包含任何业务代码。
7. **可验证交付**：项目必须能跑起来，不允许"理论上能跑"。
...
20. **严格对齐蓝图**：API 端点、请求/响应格式、DB schema 必须忠实于 `code_blueprint.md`。发现蓝图有误 → 暂停编码 → 记入发现清单 → 等问题解决后再继续。
21. **不改前端 UI**：只替换前端数据层（mock → fetch 真实 API）。页面布局/交互/样式一律不碰。
22. **逐模块推进**：一次只写 1 个 API 模块。当前模块联调通过前，不得开始下一个。
