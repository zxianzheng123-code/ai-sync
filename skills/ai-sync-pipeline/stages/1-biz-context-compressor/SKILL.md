...
- 按 `amendment_protocol.md` A0 判断，以本 skill 末尾 `## Amendment Slots` 为参数：
  - 存在 discovery → 读取 `amendment_protocol.md`，执行 A0→A5。
  - 客户要求修改 → 客户经理按 `<probe>` 追问，翻译为修订清单 → 读取 `amendment_protocol.md`，执行 A0→A5。
  - 均否 → 首次执行，继续 Step 2a。
